...
const css = `.${HIDE_PORTAL_STYLE}{position:absolute;visibility:hidden; transform:unset; pointer-events:none;width:100%;height:100%;top:0;left:0}`;
let style:HTMLStyleElement;

export class OverlayManager {
    private domMirror:DOMMirror = new DOMMirror(filterIdAndOn);

    private portalRoot:HTMLDivElement = createHTML(`<div class="${PORTAL_ROOT_CLASS} ${HIDE_PORTAL_STYLE}" data-automation-id="${PORTAL_ROOT_CLASS}"></div>`) as HTMLDivElement;
    private contentLayer:HTMLDivElement = createHTML(`<div class="${CONTENT_LAYERS_CLASS} ${HIDE_PORTAL_STYLE}" data-automation-id="${CONTENT_LAYERS_CLASS}"></div>`) as HTMLDivElement;
    private overlayLayer:HTMLDivElement = createHTML(`<div class="${OVERLAY_LAYERS_CLASS} ${HIDE_PORTAL_STYLE}" data-automation-id="${OVERLAY_LAYERS_CLASS}"></div>`) as HTMLDivElement;
    ...
    public createOverlay(overlayContext:Element):OverlayItem {
        ...
        const overlay = createHTML(`<div class="overlay ${HIDE_PORTAL_STYLE}" data-automation-id="overlay" data-overlay-id="${id}"></div>`);
        ...
        let orgClassName = overlayContext.getAttribute('class') || "";
    ...
    private mirrorParentChain(overlayContext:Element) {
        ...
        while(currentSource && currentSource !== this.root){
            ...
            if(prevMirror){
                let orgClassName = mirror.getAttribute('class')||"";
