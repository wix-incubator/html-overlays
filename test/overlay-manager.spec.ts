...
import {createHTML, OverlayManager} from '../src';
import {
    CONTENT_LAYERS_CLASS,
    HIDE_PORTAL_STYLE,
    OVERLAY_LAYERS_CLASS,
    PORTAL_ROOT_CLASS
} from '../src/overlay-manager';
...
describe('overlay manager', () => {
    ...
    describe('createOverlay()', () => {
        ...
        it('should hide and disable any effect of structural ancestors & show only the overlay target', () => {
            const root = createHTML(
                '<div><content class="a b"><div class="x y" data-portal-open="true"></div></content></div>'
            );
            const overlayContextSrc = root.querySelector('div')!;
            const om = new OverlayManager(root);

            const {layer} = om.createOverlay(overlayContextSrc);
        ...
        it('should hide and disable any effect of all ancestors', () => {
            ...
            expect((layer.firstChild!.firstChild!.firstChild! as HTMLElement).className, 'ancestor').to.contain(
                HIDE_PORTAL_STYLE
            );
            expect(
                (layer.firstChild!.firstChild!.firstChild!.firstChild! as HTMLElement).className,
                'ancestor'
    ...
    describe('destroyOverlay', () => {
        it('should remove the overlay elements', () => {
            ...
            const {destroy} = om.createOverlay(overlayContextSrc);
            destroy();
